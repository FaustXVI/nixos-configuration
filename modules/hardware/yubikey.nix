  ...
  config = {
    ...
    services = {
      udev = {
        ...
        extraRules = ''
          ACTION=="remove", ENV{KEY}=="?*", ENV{ID_BUS}=="usb", ENV{ID_MODEL_ID}=="0407", ENV{ID_VENDOR_ID}=="1050", ENV{DISPLAY}=":0.0", ENV{XAUTHORITY}="${config.users.users.xadet.home}/.Xauthority", RUN+="${pkgs.bash}/bin/bash -c 'systemctl --no-block start ${pkgs.i3lock}/bin/i3lock'"
        '';
      };
    };
