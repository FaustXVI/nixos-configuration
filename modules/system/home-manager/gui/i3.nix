...
# change focus
                 ...
                 "XF86AudioRaiseVolume" = ''exec --no-startup-id pactl set-sink-volume $(pactl info | grep -i "Sink" | cut -f2 -d:) +5%'';
                 "XF86AudioLowerVolume" = ''exec --no-startup-id pactl set-sink-volume $(pactl info | grep -i "Sink" | cut -f2 -d:) -5%'';
                 "XF86AudioMute" = ''exec --no-startup-id pactl set-sink-mute $(pactl info | grep -i "Sink" | cut -f2 -d:) toggle'';
        };
        keycodebindings = {
                # switch to workspace
